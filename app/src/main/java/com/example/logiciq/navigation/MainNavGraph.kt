...
//    }
    ...
    // Mở màn hình chia sẻ bài thi vào lớp
    composable("${Routes.SHARE}/{quizId}") { backStackEntry ->
        val quizId = backStackEntry.arguments?.getString("quizId") ?: ""
        ShareToClassScreen(navController, quizId)
    }

    // Mở TestScreen từ quizId
    composable("${Routes.TEST}/{quizId}") { backStackEntry ->
        val quizId = backStackEntry.arguments?.getString("quizId") ?: return@composable
        TestScreen(navController = navController, quizId = quizId)
    }

    // Mở ExamScreen từ quizId
    composable("${Routes.EXAM}/{quizId}") { backStackEntry ->
        val quizId = backStackEntry.arguments?.getString("quizId") ?: return@composable
        ExamScreen(navController = navController, quizId = quizId)
    }
}